...
function getEnd(clueIndex) {
  let c = puz.clues[clueIndex].cells
  return c[c.length - 1]
}
...
function join(rc1, rc2, color) {
  ...
  let col = rc1[1]
  let vert_y = -1
  let vert_x = puz.GRIDLINE + (col + 1) * (puz.squareDim + puz.GRIDLINE) - PATH_OFFSET
  let vert_height = 0
  while (row < rc2[0]) {
    const pathRect =
    ...
    pathRect.setAttributeNS(
            null, 'x',
            puz.GRIDLINE + (col + 1) * (puz.squareDim + puz.GRIDLINE) - PATH_OFFSET);
    pathRect.setAttributeNS(
            null, 'y',
            puz.GRIDLINE + (row + 1) * (puz.squareDim + puz.GRIDLINE) - PATH_OFFSET);
    pathRect.setAttributeNS(null, 'width', PATH_THICKNESS);
    pathRect.setAttributeNS(null, 'height', puz.squareDim + puz.GRIDLINE);
    pathRect.setAttributeNS(null, 'fill', color);
    path.appendChild(pathRect)
    if (vert_y < 0) {
      vert_y = puz.GRIDLINE + (row + 1) * (puz.squareDim + puz.GRIDLINE) - PATH_OFFSET
    }
    vert_height += puz.squareDim + puz.GRIDLINE
    row = row + 1
  }
  let deltax = 1
  if (col > rc2[1]) {
  ...
  let hor_x = -1
  let hor_y = puz.GRIDLINE + (row + 1) * (puz.squareDim + puz.GRIDLINE) - PATH_OFFSET
  let hor_width = 0
  while (col != rc2[1]) {
    const pathRect =
      document.createElementNS('http://www.w3.org/2000/svg', 'rect');
    ...
    pathRect.setAttributeNS(
            null, 'x',
            puz.GRIDLINE + x * (puz.squareDim + puz.GRIDLINE) - PATH_OFFSET);
    pathRect.setAttributeNS(
            null, 'y',
            puz.GRIDLINE + (row + 1) * (puz.squareDim + puz.GRIDLINE) - PATH_OFFSET);
    pathRect.setAttributeNS(null, 'height', PATH_THICKNESS);
    pathRect.setAttributeNS(null, 'width', puz.squareDim + puz.GRIDLINE);
    pathRect.setAttributeNS(null, 'fill', color);
    path.appendChild(pathRect)
    if (hor_x < 0 || deltax < 0) {
      hor_x = puz.GRIDLINE + x * (puz.squareDim + puz.GRIDLINE) - PATH_OFFSET
    }
    hor_width += puz.squareDim + puz.GRIDLINE
    col = col + deltax
  }
  // Add worm rings
...
function replaceHandlers() {
  ...
  puz.updateAndSaveState = (function() {
    ...
    return function() {
      ...
      if (lim) {
        if (hashCode(puz.answersList[0].input.value) == 72196554) {
          lim.style.display = ''
          lh.style.display = ''
        } else {
...
function customizeExolve(p) {
  ...
  lh.style.color = 'red'
  lh.style.display = 'none'
  puz.answersList[0].input.parentElement.appendChild(lh)

  puz.updateAndSaveState()
}
